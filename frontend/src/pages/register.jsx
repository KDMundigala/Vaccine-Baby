...
const RegisterPage = () => {
  ...
  const register = useAuth().register; // Assuming useAuth provides a register function
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      let profilePhotoUrl = '';
      if (formData.profilePhoto && formData.profilePhoto.name) {
        // const storageRef = ref(storage, `profile_pictures/${formData.email}/${formData.profilePhoto.name}`);
        // const uploadResult = await uploadBytes(storageRef, formData.profilePhoto);
        profilePhotoUrl = await uploadProfilePicture(formData.profilePhoto, formData.name);
        formDataToSend.append('profilePicture', profilePhotoUrl);
      }
        ...
        >
          ...
          <Box
            sx={{
              ...
              mt: { xs: '-23%', sm: '-21%', md: '-10%' }, // Added negative margin top to move the form up
            }}
          >
            <motion.div
